...
def generate_facility_capacities(M):
    ...
    while len(capacities) < M:
        random_capacity = int(random.uniform(M * 0.5, M * 1.8))
...
def plot_result(points: np.ndarray, point_allocations: dict, opt_sites: list, other_sites: list):
    ...
    """
    plt.scatter(points[:, 0], points[:, 1], c="black", s=4, alpha=0.4)
    ax = plt.gca()

    plt.scatter(other_sites[:, 0], other_sites[:, 1], color="black", s=200, marker="2")
    plt.scatter(opt_sites[:, 0], opt_sites[:, 1], color="red", s=800, marker="2")

    cycol = cycle(plt.get_cmap("tab20").colors)
    for k, v in point_allocations.items():
        color = next(cycol)
        for point_idx in v:
            plt.scatter(points[point_idx][0], points[point_idx][1], color=np.array([color]), marker="+")
